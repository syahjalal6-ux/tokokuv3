...
// Semua logic yang sebelumnya ada di src/lib/api/supabase.js
// dan memakai `supabaseAdmin` (SERVICE_ROLE_KEY) dipindahkan ke sini.
//
// Env var yang dibutuhkan di Vercel (TANPA prefix VITE_):
//   SUPABASE_URL
//   SUPABASE_SERVICE_ROLE_KEY
//
// Frontend memanggil endpoint ini lewat POST /api/admin
// dengan body: { action: "tokoApi.getMine", token, args: [...] }
// ================================================
...
// Client publik (anon) — untuk operasi yang sebelumnya pakai `supabase` biasa (bukan admin)
...
function mapPesanan(p) {
  if (!p) return null
  return {
    id: p.id, tokoId: p.toko_id, tokoSlug: p.toko?.slug || null,
...
function mapStreamPost(p, extra = {}) {
  ...
  return {
    ...
    teks: p.teks, postType: p.post_type || null, foto,
    shopLink: p.shop_link ? { slug: p.shop_link.slug, nama: p.shop_link.nama } : null,
    hashtags: extra.hashtags || [],
    likesCount: p.likes_count, repostsCount: p.reposts_count, repliesCount: p.replies_count,
  ...
  loginWithGoogle: async (googleUser) => {
    const { data, error } = await supabaseAdmin
      ...
      .upsert({
        ...
        google_id: googleUser.sub, updated_at: new Date().toISOString(),
...
const pesananApi = {
  create: async (data) => {
    const { data: pesanan, error } = await supabasePublic
      .from('pesanan')
      .insert({
        toko_id: data.tokoId, produk_id: data.produkId, produk_nama: data.produkNama,
        harga: Number(data.harga), qty: Number(data.qty), buyer_nama: data.buyerNama,
        buyer_wa: data.buyerWa, buyer_alamat: data.buyerAlamat, catatan: data.catatan || '',
        total: Number(data.total || data.harga * data.qty), status: 'pending', kurir: '', resi: '',
        created_at: new Date().toISOString(), updated_at: new Date().toISOString(),
      })
      .select()
      .single()
  ...
  getDashboard: async (token) => {
    ...
    return {
      ...
      data: {
        ...
        totalRevenue, revenueMingguIni, revenueMingguLalu,
