  ...
  create: async (token, data) => {
    ...
    const { data: bundle, error } = await supabaseAdmin
      ...
      .insert({
        ...
        nama: data.nama,
        deskripsi: data.deskripsi || '',
        harga_bundle: Number(data.hargaBundle),
        produk_ids: JSON.stringify(data.produkIds || []),
        aktif: data.aktif !== false,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
      })
...
export const flashSaleApi = {
  ...
  getActive: async (tokoId) => {
    ...
    return { success: true, data: (data || []).map(p => ({
      ...mapProduk(p),
      hargaFlash: p.harga_flash,
      flashSaleUntil: p.flash_sale_until,
    })) }
  },
}
...
export const voucherApi = {
  create: async (token, data) => {
    ...
    // Cek duplikat kode dalam toko yang sama
    const { data: existing } = await supabaseAdmin
      .from('vouchers')
      .select('id')
      .eq('toko_id', toko.id)
    ...
    const { data: voucher, error } = await supabaseAdmin
      ...
      .insert({
        ...
        terpakai: 0,
        aktif: data.aktif !== false,
        berlaku_sampai: data.berlakuSampai || null,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
      })
      .select()
  ...
  // Dipanggil buyer saat checkout — validasi + hitung diskon
  validate: async (tokoId, kode, totalBelanja) => {
    ...
    return {
      ...
      data: {
        ...
        nilai: voucher.nilai,
        diskon,
        totalSetelahDiskon: Number(totalBelanja) - diskon,
      }
    }
  },

  // Dipanggil setelah pesanan berhasil dibuat — increment terpakai
  redeem: async (voucherId) => {
    const { error } = await supabase.rpc('increment_voucher_terpakai', { p_voucher_id: voucherId })
...
function mapVoucher(v) {
  ...
  return {
    ...
    tipe: v.tipe,
    nilai: v.nilai,
    minBelanja: v.min_belanja,
    maksDiskon: v.maks_diskon,
    kuota: v.kuota,
    terpakai: v.terpakai,
    aktif: v.aktif,
